...
class _CreateOrderScreenState extends ConsumerState<CreateOrderScreen> {
  ...
  void _addProductToOrder(ProductEntity product, int quantity) {
    ...
    setState(() {
      ...
      _currentOrderItems.add(OrderItemEntity(
  ...
  void _showSelectProductDialog() {
    final productsAsyncValue = ref.read(productsListProvider); // Leer una vez para el dialogo

    productsAsyncValue.when(
      data: (products) {
        ProductEntity? selectedProduct = products.isNotEmpty ? products.first : null;
        int quantity = 1;

        showDialog(
          context: context,
          builder: (BuildContext dialogContext) {
            return StatefulBuilder( // Para actualizar el Dropdown dentro del dialogo
              builder: (context, setDialogState) {
                return AlertDialog(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        validator: (value) {
                          ...
                          if (qty == null || qty <= 0) return 'Cantidad debe ser > 0';
                          if (selectedProduct != null && qty > selectedProduct!.stock) return 'Stock insuficiente (${selectedProduct!.stock})';
                          return null;
                        },
                      ),
                    ],
                  ...
                  actions: [
                    ...
                    ElevatedButton(
                      onPressed: products.isEmpty || selectedProduct == null
                        ? null
                        : () {
                            if (quantity > 0 && selectedProduct!.stock >= quantity) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Cantidad inválida o stock insuficiente.')),
                              );
                            }
                          },
      ...
      loading: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cargando productos...'))),
      error: (err, stack) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error productos: $err'))),
    );
  }
      ...
      state.whenOrNull(
        data: (order) {
          if (order != null) {
            ...
            ref.invalidate(ordersListProvider); // Para refrescar la lista de pedidos
            Navigator.of(context).pop(); // Volver a la pantalla anterior
          }
        },
        error: (error, stackTrace) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error al crear el pedido: ${error.toString()}')),
          );
        },
        loading: () {
      ...
      body: Padding(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text('Cliente ID: $_userId (Ejemplo)', style: Theme.of(context).textTheme.titleMedium),
              ...
              const SizedBox(height: 20),
              Text('Productos en el pedido:', style: Theme.of(context).textTheme.headlineSmall),
              Expanded(
                child: _currentOrderItems.isEmpty
                    ? const Center(child: Text('Ningún producto añadido.'))
                    : ListView.builder(
                        itemCount: _currentOrderItems.length,
                        itemBuilder: (context, index) {
                          final item = _currentOrderItems[index];
                          return ListTile(
                            title: Text(item.product?.name ?? 'Producto ID: ${item.productId}'),
                            subtitle: Text('Cantidad: ${item.quantity} x \$${item.unitPrice.toStringAsFixed(2)}'),
                            trailing: IconButton(
              ...
              const SizedBox(height: 20),
              ElevatedButton(
                onPressed: createOrderState.isLoading ? null : _submitOrder,
